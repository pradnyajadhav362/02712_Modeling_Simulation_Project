...
# (current) used group centroids of the training set to calculate accuracy 16/08/2017
#
####################################################################################################
...
# Return the edge strength (vector) calculated by a linear function
# Inputs: edge features (e by w) and edge feature weights (vector w)
def ReLU_edge_strength(features, w):
...
def strength_Q_and_gradient(edges, nnodes, features, w, func='logistic'):
    ...
    if func == 'logistic':
        ...
        edge_strength = ReLU_edge_strength(features, w)
        strength_grad = features
...
def generate_Q(edges, nnodes, features, w, func='logistic'):
...
class SRW_solver(object):
    ...
    def __init__(self, edges, features, nnodes, P_init, rst_prob, group_labels, lam, w_init_sd=0.01, 
                 ...
                 early_stop=None, strength_func='logistic'):
        ...
        self.strength_func = strength_func # Edge strength function {'logistic', 'ReLU'}
        ...
        if self.loss == 'squared':
            ...
            (self.cost, cost_grad, self.accuracy, 
             M_opponent, M_opponent_trunc) = cost_func_and_grad_sil(self.nfeatures, self.nsamples, 
                                                                    ...
                                                                    self.C, C_grad, 
                                                                    self.sample2groupid_list)
            self.sil = -self.cost / self.nsamples
...
#            C = self.centroid(P, self.ngroups_val, self.group2indeces_list_val)
            ...
            self.cost_val = -self.sil_val * self.nsamples_val
...
#            C = self.centroid(P, self.ngroups, self.group2indeces_list)
            self.sil, self.accuracy = cost_func_sil(self.nsamples, self.ngroups, P, self.C, 
                                                    self.sample2groupid_list)
            self.sil = -self.sil / self.nsamples
            self.accuracy = self.accuracy / self.nsamples
        elif train_or_val == 'val':
#            C = self.centroid(P, self.ngroups_val, self.group2indeces_list_val)
            self.sil_val, self.accuracy_val = cost_func_sil(self.nsamples_val, 
                                                            self.ngroups_val, P, self.C, 
                                                            self.sample2groupid_list_val)
            self.sil_val = -self.sil_val / self.nsamples_val
    ...
    def calc_J_Jgrad_wrapper(self, w_local, t):
        ...
        print '*** silhouette is', self.sil, 'silhouette_val is', self.sil_val
        print w_local, '\n'
    ...
    def generate_Q_and_P_fin(self):
        self.Q_fin = generate_Q(self.edges, self.nnodes, self.features, self.w, self.strength_func)
