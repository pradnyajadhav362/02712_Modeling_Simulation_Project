...
# y (1, 0) or (1, -1) indicates whether the 2 samples belong to the same group,
# and z_uv is the number of sample pairs u and v belong to.
...
# v0.4.1 (current) corrected the 'silhouette' loss function, and added early stopping 08/08/2017
#
####################################################################################################
        ...
        if cost_u < 0:
            ...
            cost_grad += (-2/pu_minus_ci_sq*(P_dot_PgradT[:,u,u]-P_dot_CgradT[:,u,j]
                          ...
                          2*pu_minus_cj_sq/np.square(pu_minus_ci_sq)*(P_dot_PgradT[:,u,u]
                                                                      ...
                                                                      -C_dot_PgradT[:,i,u]
                                                                      +C_dot_CgradT[:,i,i]))
    return cost, cost_grad, accuracy
...
class SRW_solver(object):
    ...
    def __init__(self, edges, features, nnodes, P_init, rst_prob, group_labels, lam, w_init_sd=0.01, 
                 ...
                 P_init_val=None, group_labels_val=None, eval_sil=False, ncpus=-1, maxit=1000, 
                 early_stop=None):
        ...
        self.loss = loss                   # Type of the loss funtion ('squared', 'absolute')
        ...
        self.group_labels_val = group_labels_val # Group labels of validation samples (m by n, float, csr_matrix)
        self.eval_sil = eval_sil           # Whether to evaluate silhouette width (bool)
        
        # maximize_diff: Whether to maximaze the difference between groups 
        ...
        self.pair2group_args_val = None
        self.group2indeces_list_val = None
        self.sample2groupid_list_val = None
        self.cost_val_list = None
        self.w_list = None
    ...
    def obj_func_and_grad(self, P, P_grad, w_local):
        ...
        if self.loss == 'squared':
            ...
            # ||pu-ci||**2
            C, C_grad = self.centroid_and_grad(P, P_grad, self.ngroups, self.group2indeces_list)
            self.cost, cost_grad = cost_func_and_grad_centroid(self.nfeatures, self.nsamples, 
                                                               self.ngroups, P, P_grad, C, C_grad, 
                                                               self.sample2groupid_list)
        elif self.loss == 'silhouette':
            # max((||pu-ci||**2 - ||pu-cj||**2),0)
            C, C_grad = self.centroid_and_grad(P, P_grad, self.ngroups, self.group2indeces_list)
            self.cost, cost_grad, self.accuracy = cost_func_and_grad_sil(self.nfeatures, 
                                                                         self.nsamples, 
                                                                         self.ngroups, 
                                                                         P, P_grad, 
                                                                         C, C_grad, 
                                                                         self.sample2groupid_list)
            self.cost_sil = -self.cost / self.nsamples
            self.accuracy = self.accuracy / self.nsamples

        if self.norm_type == 'L2':
            # Calculate the L2 norm squared of edge feature weights
            norm = np.dot(w_local, w_local)
            norm_grad = 2 * w_local
        elif self.norm_type == 'L1':
            # Calculate the L1 norm of edge feature weights
            norm = np.sum(np.abs(w_local))
    ...
    def calc_cost_val(self, P, w_local):
        if self.loss == 'squared':
            ...
            self.cost_val = cost_func_centroid(self.nfeatures, self.nsamples_val, self.ngroups_val, 
                                               P, C, self.sample2groupid_list_val)
        elif self.loss == 'silhouette':
            self.eval_sil_wrapper(P, 'val')
            self.cost_val = -self.cost_sil_val * self.nsamples
    
    
    # This is the wrapper function for evaluating the accuracy
    def eval_sil_wrapper(self, P, train_or_val='val'):
        if issparse(P):
            P = P.toarray()
        if train_or_val == 'train':
            # Define a matrix C (g by n), which contains the group centroid
            C = self.centroid(P, self.ngroups, self.group2indeces_list)
            self.cost_sil, self.accuracy = cost_func_sil(self.nsamples, self.ngroups, P, C, 
                                                         self.sample2groupid_list)
            self.cost_sil = -self.cost_sil / self.nsamples
            self.accuracy = self.accuracy / self.nsamples
        elif train_or_val == 'val':
            C = self.centroid(P, self.ngroups_val, self.group2indeces_list_val)
            self.cost_sil_val, self.accuracy_val = cost_func_sil(self.nsamples_val, 
                                                                 self.ngroups_val, P, C, 
                                                                 self.sample2groupid_list_val)
            self.cost_sil_val = -self.cost_sil_val / self.nsamples
    ...
    def calculate_J_and_gradient(self, w_local):
        ...
        Q, M_strength, M_strength_rowSum, strength_grad = strength_Q_and_gradient(self.edges, 
                                                                                  self.nnodes, 
                                                                                  self.features, 
                                                                                  w_local)
        ...
        P = csr_matrix(iterative_PPR(Q.toarray(), renorm(self.P_init).toarray(), self.rst_prob))
        print 'finished network propagation:', time.strftime("%H:%M:%S")

        # Calculate the gradient of PPR (w by m by n)
        P_grad = calc_P_grad_pool(self.edges, self.nnodes, M_strength, M_strength_rowSum, Q, P, 
    ...
    def calc_cost_and_acc_val(self, w_local):
        if self.P_init_val is not None:
            Q = generate_Q(self.edges, self.nnodes, self.features, w_local)
            P_val = iterative_PPR(Q.toarray(), renorm(self.P_init_val).toarray(), self.rst_prob)
            self.calc_cost_val(P_val, w_local)
            if self.eval_sil:
                self.eval_sil_wrapper(P_val, 'val')
    ...
    def train_SRW_BFGS(self):
        ...
        self.calc_cost_and_acc_val(self.w)
    ...
    # This function generate after the training
