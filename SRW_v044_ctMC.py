...
def load_network(file_name, output_dir='', add_selfloop=True):
    ...
    edges = df.iloc[:,:2].apply(lambda col: col.map(node2index)).values
...
def ctmc_PPR(Q, P_init, rst_prob):
    """
    Continuous-time / steady-state implementation of Personalized PageRank.

    Parameters
    ----------
    Q : (n, n) array
        Row-stochastic transition matrix (already row-normalized).
    P_init : (k, n) or (n,) array
        Initial distribution(s). Each row is a seed distribution.
    rst_prob : float
        Restart probability α in (0, 1).

    Returns
    -------
    P_ss : array, same shape as P_init
        Steady-state Personalized PageRank vector(s).
    """
    # Ensure 2D: treat each row of P_init as one seed
    P_init = np.atleast_2d(P_init)
    n = Q.shape[0]
    ...
    # Solve A^T X^T = (alpha * P_init)^T
    # -> X = alpha * P_init @ (A^{-1})
    rhs = (alpha * P_init).T           # shape (n, k)
    P_ss_T = np.linalg.solve(A.T, rhs) # shape (n, k)
    P_ss = P_ss_T.T                    # shape (k, n)

    # If the user passed a 1D vector, return 1D
    if P_init.shape[0] == 1:
        return P_ss[0]
    return P_ss
...
def cost_func_and_grad_WMW(nfeatures, nsamples, ngroups, P, P_grad, C, C_grad, sample2groupid_list,
    ...
    for l in range(nfeatures):
        P_dot_PgradT[l,:,:] = np.dot(P, np.ascontiguousarray(P_grad[l,:,:].T))
        P_dot_CgradT[l,:,:] = np.dot(P, np.ascontiguousarray(C_grad[l,:,:].T))
        C_dot_PgradT[l,:,:] = np.dot(C, np.ascontiguousarray(P_grad[l,:,:].T))
        C_dot_CgradT[l,:,:] = np.dot(C, np.ascontiguousarray(C_grad[l,:,:].T))
    cost = 0.
...
def sample_groupid_args(group_labels):
    ...
    sample2groupid_list = np.array(sample2groupid_list, dtype=np.int64)
    group2nsamples_list = np.array(group2nsamples_list, dtype=np.float64)
...
class SRW_solver(object):
    ...
    def calculate_J_and_gradient(self, w_local):
        ...
        P = csr_matrix(ctmc_PPR(self.Q.toarray(), renorm(self.P_init).toarray(), self.rst_prob))
    ...
    def calc_cost_and_acc_val(self):
        if self.P_init_val is not None:
            P_val = ctmc_PPR(self.Q.toarray(), renorm(self.P_init_val).toarray(), self.rst_prob)
    ...
    def generate_Q_and_P_fin(self):
        ...
        self.P_fin = ctmc_PPR(self.Q_fin.toarray(), renorm(self.P_init).toarray(),
                                   self.rst_prob)
